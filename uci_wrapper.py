...
class UciWrapper:
  ...
  def set_position(self, fen: str, moves: list[str] | None = None):
    ...
    if moves:
      ...
      parts.extend(moves)
    msg = ' '.join(parts)
    print(f'uci msg: {msg!r}')
    self._process.stdin.write(msg + '\n')

  def get_best_move(self, time_limit_ms: int):
    start = time.time()
    ...
    def communicate(process, lines):
      while True:
        line = process.stdout.readline()
        print('uci line response:', line)
        lines.append(line)
        if not line or 'bestmove' in line or 'Game completed' in line:
          break

    lines = []
    t = threading.Thread(target=communicate, args=(self._process, lines))
    t.start()
    timeout_sec = (time_limit_ms + buffer_ms) / 1000.0 + 5
    t.join(timeout_sec)

    if t.is_alive():
      raise ValueError('Warning: UCI subprocess timed out')
    ...
    for line in lines:
      ...
      if 'bestmove' in line:
        ...
        return m.group(1)
    raise ValueError('Best move not found in process response:', lines)
